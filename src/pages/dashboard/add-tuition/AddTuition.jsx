...
const AddTuition = () => {
    ...
    const handleDivisionChange = (e) => {
        ...
        const selectedDivision = locationData.find(div => div.division_id === divisionId);
        setDistricts(selectedDivision ? selectedDivision.districts : []);
    };

    const onSubmit = async (data) => {
        try {
            // Find division and district names
            const divisionName = locationData.find(div => div.division_id === data.division)?.division_name;
            const districtName = districts.find(dist => dist.district_id === data.district)?.district_name;
    ...
    return (
        <div className="w-full p-4 md:p-8 bg-base-200 min-h-screen">
            <h2 className="text-3xl font-bold text-center text-primary mb-8">Add New Tuition</h2>
            
            <form onSubmit={handleSubmit(onSubmit)} className="bg-base-100 p-6 rounded-lg shadow-xl grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="form-control">
                    ...
                    <select className="select select-bordered w-full" {...register("tutorGender", { required: true })}>
